...
export const comments = createSlice({
  ...
  reducers:{
    createComment(state, action){
        ...
        axios.post("http://15.164.228.166/api/auth/comments");
    },
    removeComment(state, action){
        let index = state.comments.findIndex(comment => comment.id === action.payload);
        state.comments.splice(index,1);
        axios.delete(`http://15.164.228.166/api/auth/comments/${action.payload}`);
    },
    updateComment(state, action){
        let index = state.comments.findIndex(post => post.id === action.payload.id)
        state.comments.splice(index, 1, action.payload);
        axios.patch(`http://15.164.228.166/api/auth/comments/${action.payload.id}`, action.payload)
    }
  },
  extraReducers: {
